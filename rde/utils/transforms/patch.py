...
class FocusedRandomPatch(object):
    ...
    def __call__(self, data, dummy=None):
        ...
        seed_idx = torch.multinomial(focus_flag.float(), num_samples=1).item()  # 选定某一种子index
        ...
        nbh_seed_idx = dist_from_seed.argsort()[:self.seed_nbh_size]    # (Nb, )  选择离种子index最近的32个残基
...
class SelectedRegionFixedSizePatch(object):
    ...
    def __call__(self, data, dummy=None):
        ...
        patch_idx = torch.argsort(dist_from_sel)[:self.patch_size]   # 选择距离突变残基距离最近的128个残基作为样本
        # patch_idx = torch.cat([torch.argsort(dist_from_sel)[:9*self.patch_size//10],
        #                        torch.argsort(dist_from_sel)[9*self.patch_size//10: self.patch_size]],
        #                        dim=-1)
        patch_idx = torch.sort(patch_idx,0)[0]
        data_patch = _index_select_data(data, patch_idx)
        return data_patch, dummy
